...
void V_cal(float V1[M+1][N+1][P+1], float V2[M+1][N+1][P+1], float V3[M+1][N+1][P+1])
{
    ...
    for (step = 0; step < GAUSSIAN_STEP; step++) {
        ...
        for (counter = 0; counter < M; counter++) {
            for (j = 1; j < P; j++) {
                for (i = 0; i < N; i++) {
                    V1[counter][i][j] += V1[counter][i][j-1] * nu;
                    V2[counter][i][j] += V2[counter][i][j-1] * nu;
                    V3[counter][i][j] += V3[counter][i][j-1] * nu;
                }
            }
        }
        ...
        for (counter = 0; counter < M; counter++) {
            for (j = P-2; i >= 0; j--) {
                for (i = 0; i < N; i++) {
                    V1[counter][i][j] += V1[counter][i][j+1] * nu;
                    V2[counter][i][j] += V2[counter][i][j+1] * nu;
                    V2[counter][i][j] += V3[counter][i][j+1] * nu;
                }
            }
        }
        ...
        for (counter = 0; counter < M; counter ++) {
            for (i = 1; i < N; i++) {
                for (j = 0; j < P; j++) {
                    V1[counter][i][j] += V1[counter][i-1][j] * nu;
                    V2[counter][i][j] += V2[counter][i-1][j] * nu;
                    V3[counter][i][j] += V3[counter][i-1][j] * nu;
                }
            }
        }
        ...
        for (counter = 1; counter < M; counter++) {
            for (i = 0; i < N; i++) {
                for (j = 0; j < P; j++) {
                    V1[counter][i][j] += V1[counter-1][i][j] * nu;
                    V2[counter][i][j] += V2[counter-1][i][j] * nu;
                    V3[counter][i][j] += V3[counter-1][i][j] * nu;
                }
            }
        }
